...
?>
    ...
    ?>
    <div class="alert alert-warning alert-dismissible fade show" id="warningAlertfila" style="position: fixed; top: 20px; right: 20px; z-index: 1050; display: none;" role="alert">
        <strong>¡Advertencia!</strong> Por favor, selecciona al menos una fila.
    </div>
    <div class="container d-flex justify-content-between align-items-start" style="height: 110vh; margin-top:20px;">
        <!-- Tabla de IES PARTICIPAN -->
        <div class="card me-4" style="width: 1500px; max-width: 1800px; margin-left:-70px; margin-right:30px;">
            <h6 class="card-header d-flex justify-content-between align-items-center">
                <span>IES PARTICIPAN (<span id="iesParticipanCount"><?= $data['totalParticipan'] ?></span>)</span>
                <!-- buscador -->
                <div class="mb-3" style="width: 220px;">
                    <label for="searchCodModular" class="form-label">Buscar por Código Modular:</label>
                    <input type="text" class="form-control" id="searchCodModular" placeholder="Ingrese el Código Modular">
                </div>
                ...
                <div class="panel panel-default">
                    <div class="panel-body table-responsive">
                        <table class="table table-bordered" style="font-size: 13px;" id="iesParticipanTable">
                            <thead>
                                <tr>
                                    <th><input type='checkbox' id='select-all-participan' onchange="toggleCheckboxes(this, 'iesParticipanTable')"></th>
                                    <th>#</th>
                            ...
                            <tbody>
                                ...
                                if (is_array($data['iesParticipan'])) {
                                    ...
                                    foreach ($data['iesParticipan'] as $ie) : ?>
                                        <tr>
                                            <td class="col-xs-2">
                                                <input type='checkbox' class='select-row' data-codmodular="<?= htmlspecialchars($ie['codmodular']) ?>">
                                            </td>
        ...
        <center>
            <button class="btn btn-primary btn-sm" style="margin-top: 250px;" type="button" onclick="copiarDatos()">
                <span class="fw-bold">>></span>
            </button>
            <button class="btn btn-primary btn-sm" style="margin-top: 20px;" type="button" onclick="copiarDatosR()">
                <span class="fw-bold">
                    << </span>
            </button>
        </center>
        <!-- Tabla de IES que NO PARTICIPAN -->
        <div class="card me-4" style="width: 1500px; max-width: 1800px; margin-right:-70px; margin-left:30px;">
            <h6 class="card-header d-flex justify-content-between align-items-center">
                <!-- Texto del encabezado -->
                <span>IES que no participan (<span id="iesNoParticipanCount"><?= $data['totalNoParticipan'] ?></span>)</span>
                <!-- Formulario de búsqueda -->
                <div class="mb-3 ms-auto">
                    <label for="searchCodModularNoParticipan" class="form-label" style="margin-right: 10px;">Buscar por Código Modular:</label>
                    <input type="text" class="form-control" id="searchCodModularNoParticipan" placeholder="Ingrese el Código Modular">
                </div>
            </h6>
            <div class="card-body">
                <div class="panel panel-default">
                    <div class="panel-body table-responsive">
                        <table class="table table-bordered" id="iesNoParticipanTable" style="font-size: 13px;">
                            <thead>
                                <tr>
                                    <th><input type="checkbox" id="select-all-no-participan" onchange="toggleCheckboxesdos(this, 'iesNoParticipanTable')"></th>
                                    <th>#</th>
                                    <th>CODIGO MODULAR</th>
                                    <th>DESCRIPCION</th>
                                    <th>DISTRITO</th>
                            ...
                            <tbody>
                                ...
                                if (is_array($data['iesNoParticipan'])) {
                                    ...
                                    foreach ($data['iesNoParticipan'] as $ie) : ?>
                                        <tr>
                                            <td><input type='checkbox' class='select-row' data-codmodular="<?= htmlspecialchars($ie['codmodular']) ?>"></td>
                                            <td class="row-number"><?= $i++ ?></td>
                                            <td><?= $ie['codmodular'] ?> </td>
                                            <td><?= $ie['descripcion'] ?></td>
                                            <td><?= $ie['distrito'] ?></td>
